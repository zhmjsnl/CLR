...
public:
    ...
    double seekUsingLKH(treeNode* rootNode,double** weight,bool containRoot,vector<vector<int>>& tourAns,int depNum);
    double seek(int num, vector<vector<int> >& mst_info, double** weight,vector<int>& tourAns);
    double seekUsingLKH(int num, vector<vector<int> > &mst_info, double **weight, vector<int> &tourAns);
};
