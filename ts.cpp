    ...
    while (!tree->child.empty()) {
        ...
        if (capacity >= factoryNode->totalDemand) {
            ...
            if (!factoryNode->child.empty())
                if (constant::usingLKH) totalCost += t.seekUsingLKH(factoryNode, dist, containRoot, tourAns, depNum);
                else totalCost += t.seek(factoryNode, dist, containRoot, tourAns, depNum);
...
//                    cout << "minimal ST's root is factoryNode" << endl;
                ...
                if (constant::usingLKH)totalCost += t.seekUsingLKH(newRootNode, dist, containRoot, tourAns, depNum);
                else totalCost += t.seek(newRootNode, dist, containRoot, tourAns, depNum);
