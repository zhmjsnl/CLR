...
void ps::solve(int &n, int &cliNum, int &depNum, int &capacity,
    ...
    double tspCost;
    if (constant::psLKH) tspCost = tourSeek.seekUsingLKH(cliNum + 1, mst_info, distInH, tspAns);
    else tspCost = tourSeek.seek(cliNum + 1, mst_info, distInH, tspAns);
    ...
    while (!tree->child.empty()) {
        ...
        if (capacity >= factoryNode->totalDemand) {
            if (!factoryNode->child.empty())
                if (constant::usingLKH)totalCost += t.seekUsingLKH(factoryNode, dist, true, tourAns, depNum);
                else totalCost += t.seek(factoryNode, dist, true, tourAns, depNum);
            ...
            if (constant::usingLKH)
                totalCost += t.seekUsingLKH(newRootNode, dist, demandDelta != 0 && !rootIsFactory, tourAns, depNum);
            else totalCost += t.seek(newRootNode, dist, demandDelta != 0 && !rootIsFactory, tourAns, depNum);
